...
#             activation, cen = box_mode
            if box_mode is None:
                activation = 'none'
                cen = 0.02
            elif isinstance(box_mode, (tuple, list)) and len(box_mode) == 2:
                activation, cen = box_mode
            else:
        ...
        if self.geo == 'box':
            ...
            self.projection_net = BetaProjection(self.entity_dim * 2, 
                                             ...
                                             num_layers)
        self.negative_sample_regular = torch.arange(nentity).to(self.device)

    def forward(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict=None):
        if self.geo == 'box':
...
#                                 result_message = f"{k} : {id2ent[k]} : {ranking[0][k].cpu().numpy()} : {negative_logit[0][k].cpu().numpy()}"
                                result_message = f"{k} : {id2ent[k]} : {ranking[0][k].cpu().numpy()}"
...
#                     print(num_hard)
                ...
                step += 1
        output_file_path = "validation_results.txt"
